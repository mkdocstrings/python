...
- **`do_source_location`** – Get the file path displayed in an object's source block label.
...
```python
do_source_location(obj: Object | Alias) -> Path
```

Get the file path displayed in an object's source block label.

Environment paths are never displayed: when the object's file lives in a `site-packages` directory (for example a virtual environment inside the current working directory), the path below `site-packages` is returned instead.

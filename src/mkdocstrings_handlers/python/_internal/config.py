...
#   See `PythonOptions.combine()` and `PythonHandler.get_options()`.
...
class ChainedOptions:
    ...
    def chain(self, options: dict[str, Any] | PythonOptions) -> ChainedOptions:
        """Chain the current options with the given options."""
        options = options if isinstance(options, PythonOptions) else PythonOptions.from_data(**options)
        return ChainedOptions(self, options)
...
class PythonOptions(PythonInputOptions):  # type: ignore[override,unused-ignore]
    ...
    def chain(self, options: Mapping[str, Any] | PythonOptions) -> ChainedOptions:
        ...
        #     return PythonOptions.from_data(**(getattr(self, "_inputs", {}) | options))
        # if self.members is not None:
        #     return PythonOptions.from_data(**getattr(self, "_inputs", {}))
        # return self
        options = options if isinstance(options, PythonOptions) else PythonOptions.from_data(**options)
        return ChainedOptions(self, options)
